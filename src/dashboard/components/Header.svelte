<script lang="ts">
  let { currentFilter, onFilterChange, currentSort, onSortChange, searchQuery, onSearchChange, onToggleSidebar, isSidebarCollapsed } = $props<{
...
<header class="sticky top-0 bg-[var(--bg-base)] z-[40] p-4 md:p-6 lg:px-12 flex flex-col md:flex-row justify-between md:items-center border-b border-[var(--border-subtle)] gap-4 md:gap-5 transition-theme">
  <div class="flex items-center gap-4 justify-between md:justify-start w-full md:w-auto shrink-0">
    <div class="flex items-center gap-4">
      ...
      <h1 class="font-serif text-2xl lg:text-4xl text-[var(--text-primary)] tracking-tight whitespace-nowrap">
  ...
  <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
    <div class="relative w-full sm:flex-1 md:w-48 lg:w-64">
      <svg class="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text-tertiary)] w-4 h-4 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
      <input 
        type="search" 
    ...
    <div class="relative w-full sm:w-auto md:w-40 lg:w-48">
      ...
      <svg class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--text-tertiary)]" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="6 9 12 15 18 9"></polyline></svg>
    </div>
  </div>
</header>
